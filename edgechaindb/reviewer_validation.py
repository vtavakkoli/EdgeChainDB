...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run CloudCom reviewer-requested validation experiments"
    )
    parser.add_argument(
        "--profile",
        choices=("smoke", "paper"),
        default="smoke",
        help="smoke is CI-friendly; paper runs the full camera-ready experiment matrix",
    )
    parser.add_argument(
        "--result-dir",
        default="result/reviewer-validation",
        help="directory for JSON, CSV, and HTML artifacts",
    )
    ...
    try:
        for spec in _specs(args.profile, gateway):
            try:
