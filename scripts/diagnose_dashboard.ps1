...
if (($rendered -notmatch "edgechaindb:0.8.3") -or ($rendered -notmatch "edgechaindb.gateway_server")) {
    Write-Host "FAIL Compose is not using the 0.8.3 module entrypoint" -ForegroundColor Red
    $failures++
} else {
    Write-Host "PASS image edgechaindb:0.8.3 and python -m gateway entrypoint" -ForegroundColor Green
}
